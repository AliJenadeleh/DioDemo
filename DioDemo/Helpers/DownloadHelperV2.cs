...
public static class DownloadHelperV2
{
    /*
    ...
    public static async Task<string> DownloadConsortiumAsync(string ConsortiumId, int PageSize = 10000, CancellationToken cancellationToken = default)
    {
        return await DownloadAsync($"https://api.datacite.org/providers?consortium-id={ConsortiumId}&page[size]={PageSize}", cancellationToken);
    }
